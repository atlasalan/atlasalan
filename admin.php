...
function isSetup() { return file_exists(CONFIG_FILE); }
...
function saveGames($games) {
    ...
    file_put_contents(GAMES_JSON, json_encode(array_values($games), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
}
...
if (!isSetup()) {
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='setup') {
        $pw  = $_POST['password'] ?? '';
        $pw2 = $_POST['password2'] ?? '';
        if (strlen($pw) < 6) { $msg='❌ Şifre en az 6 karakter olmalı.'; $msgType='error'; }
...
// ── DASHBOARD ───────────────────────────────────────────────────
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='change_password') {
        ...
        elseif (strlen($new)<6) { $msg='❌ Yeni şifre en az 6 karakter.'; $msgType='error'; }
        elseif ($new!==$new2) { $msg='❌ Yeni şifreler eşleşmiyor.'; $msgType='error'; }
        else {
            $hash=password_hash($new,PASSWORD_BCRYPT);
            file_put_contents(CONFIG_FILE,"<?php\nreturn ['password_hash'=>'$hash'];\n");
            $msg='✅ Şifre değiştirildi.'; $msgType='success';
        }
    }
    ...
    if (isset($_GET['delete'])) {
        ...
        foreach ($games as $i=>$g) {
            if ($g['id']===$delId) {
                if (!empty($g['file'])&&file_exists(GAMES_DIR.$g['file'])) @unlink(GAMES_DIR.$g['file']);
                if (!empty($g['thumb'])&&file_exists(GAMES_DIR.$g['thumb'])) @unlink(GAMES_DIR.$g['thumb']);
    ...
    if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action']??'')==='upload') {
        ...
        elseif (!isset($_FILES['gamefile'])||$_FILES['gamefile']['error']!==UPLOAD_ERR_OK) { $msg='❌ Dosya seçilmedi.'; $msgType='error'; }
            ...
            if (!in_array($ext,$allowed)) { $msg='❌ Geçersiz tür: '.implode(', ',$allowed); $msgType='error'; }
                ...
                $fn=$id.'_'.$safe.'.'.$ext; move_uploaded_file($file['tmp_name'],GAMES_DIR.$fn);
                $tn='';
                if (isset($_FILES['thumb'])&&$_FILES['thumb']['error']===UPLOAD_ERR_OK) {
                    ...
                    if (in_array($te,['jpg','jpeg','png','gif','webp'])) {
                        $tn=$id.'_thumb.'.$te; move_uploaded_file($tf['tmp_name'],GAMES_DIR.$tn);
                    }
                }
                $games=getGames();
...
.toggle-pw{position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:1.1rem;color:var(--muted);transition:color 0.2s;padding:4px;}
...
.auth-btn{width:100%;padding:15px;border:none;border-radius:14px;cursor:pointer;font-family:'Nunito',sans-serif;font-weight:800;font-size:1rem;background:linear-gradient(135deg,var(--purple),var(--pink));color:#fff;transition:transform 0.2s,box-shadow 0.2s;margin-top:6px;}
.auth-btn:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(155,93,229,0.35);}
...
.hbtn.logout:hover{background:rgba(255,85,85,0.1);border-color:var(--red);}
...
.sbtn{width:100%;padding:14px;border:none;border-radius:12px;cursor:pointer;font-family:'Nunito',sans-serif;font-weight:800;font-size:0.95rem;background:linear-gradient(135deg,var(--purple),var(--pink));color:#fff;transition:transform 0.2s,box-shadow 0.2s;}
.sbtn:hover{transform:translateY(-2px);box-shadow:0 10px 25px rgba(155,93,229,0.3);}
    ...
    <form method="POST">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <input type="password" name="password" id="pw1" placeholder="En az 6 karakter" autocomplete="new-password" oninput="checkStr(this.value)">
      ...
      <div class="form-group">
        ...
        <div class="input-wrap">
          <input type="password" name="password2" id="pw2" placeholder="Aynı şifreyi gir" autocomplete="new-password">
          <button type="button" class="toggle-pw" onclick="togglePw('pw2',this)">👁</button>
        </div>
      </div>
      <button type="submit" class="auth-btn">🔐 Şifreyi Kaydet &amp; Giriş Yap</button>
    </form>
    <div class="hint">Bu sayfa sadece <strong>bir kez</strong> görünür.<br>Şifreni güvenli bir yere not al!</div>
  </div>
</div>
...
<div class="auth-wrap">
  <div class="auth-card">
    <div class="auth-logo">🎮 atlasalan.fun</div>
    <div class="auth-sub">Admin Paneli — Giriş</div>
    <?php if($msg):?><div class="msg <?=$msgType?>"><?=$msg?></div><?php endif;?>
    <form method="POST">
      ...
      <div class="form-group">
        <label>Şifre</label>
        <div class="input-wrap">
          <input type="password" name="password" id="pwL" placeholder="Admin şifreni gir" autofocus autocomplete="current-password">
          <button type="button" class="toggle-pw" onclick="togglePw('pwL',this)">👁</button>
        </div>
      </div>
      <button type="submit" class="auth-btn">🔓 Giriş Yap</button>
    </form>
    <div class="hint">Şifreni mi unuttun? Hosting'inden <strong>config.php</strong>'yi sil,<br>tekrar <strong>/admin.php</strong>'ye git ve yeniden kur.</div>
  </div>
</div>
...
<header>
  ...
  <div>
    ...
    <a href="admin.php?logout=1" class="hbtn logout" onclick="return confirm('Çıkış yapılsın mı?')">🚪 Çıkış</a>
  </div>
</header>

<div class="page">
  <div class="stats-row">
    <div class="stat-card"><div class="stat-num y"><?=count($games)?></div><div class="stat-lbl">🎮 Toplam Oyun</div></div>
    ...
    <div class="card">
      ...
      <div class="tab-content active" id="tab-upload">
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="fg">
            ...
            <div class="file-drop">
              <input type="file" name="gamefile" accept=".html,.htm,.exe,.zip" required onchange="setFN(this,'gfn')">
              <span class="fi">📁</span><div class="ft">Sürükle veya tıkla</div>
              <div class="file-name" id="gfn"></div>
            </div>
          </div>
          <div class="fg">
            <label>Kapak Görseli (opsiyonel)</label>
            <div class="file-drop">
              <input type="file" name="thumb" accept="image/*" onchange="setFN(this,'tfn')">
              <span class="fi">🖼️</span><div class="ft">JPG, PNG, GIF, WEBP</div>
              <div class="file-name" id="tfn"></div>
            </div>
          </div>
          <button type="submit" class="sbtn">⬆ Yükle</button>
        </form>
      </div>
      ...
      <div class="tab-content" id="tab-password">
        <form method="POST">
          ...
          <div class="fg"><label>Mevcut Şifre</label>
            <div class="input-wrap"><input type="password" name="old_password" id="p0" placeholder="Mevcut şifren"><button type="button" class="toggle-pw" onclick="togglePw('p0',this)">👁</button></div>
          </div>
          <div class="fg"><label>Yeni Şifre</label>
            <div class="input-wrap"><input type="password" name="new_password" id="p1" placeholder="En az 6 karakter" oninput="checkStr(this.value)"><button type="button" class="toggle-pw" onclick="togglePw('p1',this)">👁</button></div>
            <div class="pw-strength"><div class="pw-strength-bar" id="sBar"></div></div>
          </div>
          <div class="fg"><label>Yeni Şifre (tekrar)</label>
            <div class="input-wrap"><input type="password" name="new_password2" id="p2" placeholder="Tekrar gir"><button type="button" class="toggle-pw" onclick="togglePw('p2',this)">👁</button></div>
          </div>
          <button type="submit" class="sbtn green">🔑 Şifreyi Değiştir</button>
        </form>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <?php else:?>
        <div class="game-list">
          ...
          <div class="game-row">
            <div class="row-thumb">
              <?php if(!empty($g['thumb'])):?><img src="games/<?=htmlspecialchars($g['thumb'])?>" alt="">
            ...
            <div class="row-btns">
              ...
              <a href="admin.php?delete=<?=urlencode($g['id'])?>" class="rbtn del"
                onclick="return confirm('<?=addslashes($g['title'])?> silinsin mi?')">🗑</a>
            </div>
          </div>
          <?php endforeach;?>
        </div>
